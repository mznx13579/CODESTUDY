...
/*void input_mode_small() {
	...
	if(key_count == 1){
		if( sel.led  == 1 ) { led_clear(); led_bit_small(key_value); }
		if( sel.dot  == 1 ) { dot_clear(); dot_write(key_value); }
		if( sel.fnd  == 1 ) { fnd_clear(); fnd_write(key_value, 7); }
		if( sel.clcd == 1 ) { 
			sprintf(clcd_str, "This is %c", (key_value+97));
			clcd_set_DDRAM(0x40);
			clcd_write_string(clcd_str);
		}
	}
...
void input_mode_big() {
	...
	if(key_count == 1){
		if( sel.led  == 1 ) { led_clear(); led_bit_big(key_value); }
		if( sel.dot  == 1 ) { dot_clear(); dot_write(key_value); }
		if( sel.fnd  == 1 ) { fnd_clear(); fnd_write(key_value, 0); }
		if( sel.clcd == 1 ) { 
			sprintf(clcd_str, "This is %c", (key_value+65));
			clcd_set_DDRAM(0x40);
			clcd_write_string(clcd_str);
		}
	}
